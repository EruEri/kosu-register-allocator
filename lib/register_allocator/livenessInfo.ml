...
module Make (OrderedType : OrderedType) = struct
  ...
  let set_alive (elt : OrderedType.t) info : liveness_info =
    info
    |> List.map (fun e ->
  ...
  let alive_elements : liveness_info -> OrderedType.t list =
    List.filter_map (fun (elt, alive) -> if alive then Some elt else None)

  let set_dead (elt : OrderedType.t) info : liveness_info =
    info
    |> List.map (fun e ->
           let in_element, _ = e in
